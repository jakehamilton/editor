...
(fn on-attach [bufnr]
	...
	(keymap :s api.node.open.vertical {:desc "Open (Vertical)" :buffer bufnr :noremap true})
	(keymap :S api.node.open.horizontal {:desc "Open (Horizontal)" :buffer bufnr :noremap true})
	(keymap :t api.node.open.t {:desc "Open (Tab)" :buffer bufnr :noremap true})
	(keymap :O api.node.run.system {:desc "Open (System)" :buffer bufnr :noremap true})
...
; (keymap :<c-n> :<cmd>NvimTreeToggle<cr> {:mode :nvt :desc "Toggle Files" :noremap true})
